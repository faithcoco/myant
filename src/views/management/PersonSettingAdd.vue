      ...
      <a-card>
        <a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
          ...
          >
            <!--文本框-->
            <a-input
              v-if="item.fieldentertype == '字符'"
              v-model="form[item.fieldname.toLowerCase()]"
              placeholder=""
              :disabled="item.fieldedit == 1 ? false : true"
            ></a-input>
            <!--时间-->
            <a-date-picker
              v-if="item.fieldentertype == '时间'"
              placeholder="请选择日期"
              v-model="form[item.fieldname.toLowerCase()]"
              :disabled="item.fieldedit == 1 ? false : true"
              style="width: 100%"
...
export default {
  ...
  methods: {
    ...
    initForm() {
      ...
      if (
        this.$route.query.personid != null &&
        this.$route.query.personid != '' &&
        this.$route.query.personid != undefined
      ) {
        const params = {}
  ...
  // 初始化加载
  activated() {
   console.log("add isrun ")
  },
  created(){
console.log("created isrun ")
